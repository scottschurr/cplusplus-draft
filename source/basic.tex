...
outside of a discarded statement\iref{stmt.if};
\ifndr{at least one definition of everything odr-used is required}{no diagnostic} required.
...
\end{itemize}
the program is ill-formed;
\ifndr{one item with multiple definitions in different translation units}{a diagnostic}
is required only
...
the program is ill-formed (
\ifndr{one item with multiple definitions in different translation units}{no diagnostic}
required) because
...
those unnamed enumeration types shall be the same;
\ifndr{multiple unnamed enumeration definitions with the same first enumerator name}{no diagnostic}
required.
\indextext{one-definition rule|)}
...
the program is ill-formed,
\ifndr{invalid declaration set}{no diagnostic} required.
\begin{example}
\begin{codeblock}
...
\ifndr{two declarations of the same entity attached to different modules}{no diagnostic}
is required if neither is reachable from the other.
\begin{example}
...
\ifndr{multiple declarations for the same item in different modules}{No diagnostic}
is required if neither declaration is reachable from the other.
\begin{example}
\begin{codeblock}
